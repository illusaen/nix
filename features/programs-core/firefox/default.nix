...
in {
  modules.nixos = {
    ...
    inherit (firefoxConfig) userChrome userContent extensions searchEngines;
  in {
    config = {
      ...
      hjem.users.${user.name}.xdg.config.files = mkHjem {inherit lib pkgs sources userChrome userContent;};
    };
  };
  ...
  modules.darwin = {
    ...
    firefoxConfig = import ./config.nix {inherit pkgs;};
    inherit (firefoxConfig) userChrome userContent;
  in {
